...
The Android Phase B MVP was manually validated on the OnePlus test device: unlock and eligible-app triggers, notification delivery, the optional reminder overlay, direct study opening, the three/five-card limits, abandoned sessions, and FSRS reuse work as configured. The usage duration is configurable, defaults to three minutes and can be reduced to one minute for testing. A force-stop requires reopening Vocabulary to reactivate the service; reboot, OEM battery restrictions and production store-policy behavior remain follow-up validation.
