...
The main note type is `Basic Quizlet Extended`. Its fields are `FrontText`, `FrontAudio`, `BackText`, `BackAudio`, `Image` and `Add Reverse`, with two templates named `Normal` and `Reverse`. The importer should preserve the text fields and resolve referenced media where present, while treating the template's `Add Reverse` field as a direction-generation hint rather than as vocabulary content. The package also demonstrates that modern Anki stores note types, fields, templates and deck metadata in dedicated tables instead of the legacy JSON columns in `col`.

The supplied `Ndls 5eme__décembre__Meest gebruikte woorden - C.apkg` confirms that this is a common collection shape rather than an edge case: it contains 24 notes, 48 cards, 247 review-log entries and the same `Basic Quizlet Extended` note type with `Normal` and `Reverse` templates. Its cards include Anki FSRS data and both directions have separate scheduling state. The importer must therefore support this note type in its first useful slice. This particular archive has an empty media manifest despite the media-oriented fields, so it is useful for testing text, templates and progress but cannot validate media transfer.
