...
The current Android spike uses an opt-in notification prototype. It stores a user-triggered snapshot of available new/today cards, sends a three-card notification after `USER_PRESENT`, and checks for three minutes of eligible foreground usage before sending a five-card notification. It opens the app home screen rather than forcing an activity over another app. This remains a feasibility implementation until background delivery and OEM restrictions are validated on the test device.
