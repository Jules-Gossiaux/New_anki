...
The current Android spike uses an opt-in notification prototype. It stores a user-triggered snapshot of available new/today cards and starts a visible Android foreground service while enabled. The service sends a three-card notification after `USER_PRESENT` and checks eligible foreground usage once per second in test mode before sending a five-card notification after ten continuous seconds. Notification taps open the app home screen rather than forcing an activity over another app; no study screen is opened automatically. The intended production threshold remains three minutes. This remains a feasibility implementation until background delivery, OEM restrictions and Google Play foreground-service policy are validated on the test device.
