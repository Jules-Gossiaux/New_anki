...
The current Android spike uses an opt-in notification prototype. It stores a user-triggered snapshot of available new/today cards and starts a visible Android foreground service while enabled. The service polls Usage Access events once per second, uses the observed `KEYGUARD_HIDDEN` event for each unlock, and checks eligible foreground usage in test mode before sending a five-card notification after ten continuous seconds. Leaving an eligible app clears its test session; reopening it from Android's recent-app list starts a fresh one. Notification taps open the app home screen rather than forcing an activity over another app; no study screen is opened automatically. The intended production threshold remains three minutes. This was validated on the OnePlus test device while the app is backgrounded, but force-stop, reboot, OEM restrictions and Google Play foreground-service policy remain feasibility work.

The next intervention-study slice uses all decks by default, with an optional global priority deck that takes precedence when selected. A trigger starts a session with up to three cards after unlock or up to five cards after eligible app use; if fewer eligible cards are available, it uses the smaller number. Leaving a session abandons that session, and the next trigger starts a new one. The user may choose a notification-driven flow or an automatic in-app flow, but Android background activity-launch restrictions mean the latter is only guaranteed while Vocabulary is already foregrounded; otherwise the implementation must fall back to a notification.
