...
The current Android spike uses an opt-in notification prototype. It stores a user-triggered snapshot of available new/today cards, sends a three-card notification after `USER_PRESENT` while the app process is alive, and checks eligible foreground usage with a non-exact ten-second alarm in test mode before sending a five-card notification. Notification taps open the app home screen rather than forcing an activity over another app; no study screen is opened automatically. The intended production threshold remains three minutes. This remains a feasibility implementation until background delivery and OEM restrictions are validated on the test device.
