...
Schema/migrations, repositories, nested decks, notes/cards, validation, pinned FSRS integration, review history and the first study use case are implemented. Global revision settings, full note editing, two fixed templates, tag UX and staged `.apkg` import remain open. `.apkg` export is deliberately deferred until after the import and core editing work.
